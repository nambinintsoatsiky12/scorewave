...
/* ===== OUTILS IA PREMIUM (offres payantes CV Pro) =====
   Lettre 100% personnalisée · Simulateur d'entretien · CV en anglais */
const RL = new Map();
const rlOK = ip => {
  const k = (ip || "?") + "|" + new Date().toISOString().slice(0, 10);
  const n = (RL.get(k) || 0) + 1; RL.set(k, n);
  if (RL.size > 4000) RL.clear();
  return n <= 30;
};
const geminiText = async prompt => {
  ...
  const d = await r.json();
  if (d.error) throw new Error(d.error.message || "IA indisponible");
  const t = (d.candidates?.[0]?.content?.parts || []).map(p => p.text).join("");
  if (!t) throw new Error("Réponse vide de l'IA");
  return t;
};

app.post("/api/lettre-ia", async (req, res) => {
  try {
    if (!KEY) return res.status(500).json({ ok: false, error: "Serveur non configuré." });
    if (!rlOK(req.ip)) return res.status(429).json({ ok: false, error: "Quota du jour atteint (30 utilisations). Réessaie demain 🌙" });
    const { offre, cv } = req.body || {};
    if (!offre || String(offre).trim().length < 10)
      return res.status(400).json({ ok: false, error: "Colle d'abord l'annonce du poste (quelques lignes)." });
    const c = cv || {};
    const jour = new Date().toLocaleDateString("fr-FR", { timeZone: "Indian/Antananarivo", day: "numeric", month: "long", year: "numeric" });
    const prompt = `Tu es un expert en recrutement à Madagascar. Rédige une LETTRE DE MOTIVATION en français, convaincante et 100 % personnalisée, en réponse à cette annonce :\n« ${String(offre).slice(0, 1500)} »\n\nCandidat : ${c.name || "(nom non précisé)"} — poste actuel/visé : ${c.role || "—"} — ville : ${c.city || "Madagascar"}.\nRésumé du profil : ${c.summary || "—"}\nCompétences : ${c.skills || "—"}\n\nRègles : 220 à 320 mots ; commence par la ville et « le ${jour} » ; « À l'attention du responsable du recrutement » si l'entreprise n'est pas nommée ; 3 à 4 paragraphes (accroche liée à l'annonce, atouts concrets du candidat en lien avec le poste, motivation et disponibilité) ; formule de politesse correcte ; signature avec le nom du candidat. TEXTE BRUT uniquement : aucun markdown, aucune étoile, aucun crochet [ ] — n'invente aucune information non fournie.`;
    res.json({ ok: true, text: (await geminiText(prompt)).trim() });
  } catch (e) { res.status(502).json({ ok: false, error: "IA indisponible — réessaie dans 1 minute ☕" }); }
});

app.post("/api/entretien-ia", async (req, res) => {
  try {
    if (!KEY) return res.status(500).json({ ok: false, error: "Serveur non configuré." });
    if (!rlOK(req.ip)) return res.status(429).json({ ok: false, error: "Quota du jour atteint (30 utilisations). Réessaie demain 🌙" });
    const { poste, history } = req.body || {};
    const hist = Array.isArray(history) ? history.slice(-12) : [];
    const nA = hist.filter(h => h.role === "a").length;
    const prompt = `Tu es un recruteur expérimenté à Madagascar menant un entretien d'embauche pour le poste : « ${String(poste || "employé polyvalent").slice(0, 120)} ».\nHistorique (Q = tes questions, R = réponses du candidat) :\n${hist.map(h => (h.role === "q" ? "Q: " : "R: ") + String(h.t).slice(0, 600)).join("\n") || "(début de l'entretien)"}\n\n${nA >= 5
      ? `L'entretien est TERMINÉ. Donne le bilan final en commençant exactement par "### Bilan" : note globale /10, 3 points forts ✔, 3 axes de progrès 💡, 1 question piège à réviser, et un mot d'encouragement.`
      : nA === 0
        ? `Commence l'entretien : accueille le candidat en une phrase chaleureuse puis pose la PREMIÈRE question (« Présentez-vous en quelques mots », adaptée au poste).`
        : `Le candidat vient de répondre. Écris : 1) "✔ Feedback :" 2-3 phrases (un point fort + un conseil concret), 2) "➡ Question :" la question suivante, de plus en plus piégeuse (questions réellement posées à Madagascar).`}\nFrançais simple, ton professionnel mais bienveillant, réponse courte (max 120 mots), pas de liste sauf pour le bilan.`;
    res.json({ ok: true, text: (await geminiText(prompt)).trim(), termine: nA >= 5 });
  } catch (e) { res.status(502).json({ ok: false, error: "IA indisponible — réessaie dans 1 minute ☕" }); }
});

app.post("/api/traduis-cv", async (req, res) => {
  try {
    if (!KEY) return res.status(500).json({ ok: false, error: "Serveur non configuré." });
    if (!rlOK(req.ip)) return res.status(429).json({ ok: false, error: "Quota du jour atteint (30 utilisations). Réessaie demain 🌙" });
    const { cv } = req.body || {};
    if (!cv) return res.status(400).json({ ok: false, error: "CV vide." });
    const prompt = `Translate this résumé content from French to PROFESSIONAL ENGLISH (international CV style). Reply with ONLY valid JSON, exactly the same structure, keys and array lengths as the input. Translate job titles and degree names too. Do NOT translate proper nouns (people, companies, schools, cities).\n\nINPUT JSON:\n${JSON.stringify(cv).slice(0, 6000)}`;
    const t = await geminiText(prompt);
    let out = null;
    try {
      const clean = t.replace(/```[a-z]*/gi, "").trim();
      out = JSON.parse(clean.slice(clean.indexOf("{"), clean.lastIndexOf("}") + 1));
    } catch { return res.json({ ok: false, error: "Traduction illisible — réessaie." }); }
    res.json({ ok: true, cv: out });
  } catch (e) { res.status(502).json({ ok: false, error: "IA indisponible — réessaie dans 1 minute ☕" }); }
});
